              ...
              <span className="close" onClick={this.closeModal}>
                ...
                <div className="modal-body">
                  <p>
                    ...
                    blue team agent, red team agent, a civilian, or the
                    assassin.
                    The game is played with two teams, a red team and a blue
                    team.
                    Both teams have one spymaster and one detective.
                    <br/>
                    <br/>
                    At the beginning of the game, only the spymasters can see
                    the
                    position of the agents (this can be viewed by clicking on
                    the
                    key button). The spymasters will each take turns giving
                    clues
                    to their team's detective on which agents belong to their
                    team. These clues will include a one word hint and the
                    amount
                    of agents that the hint relates to. The detective will then
                    guess which word the clue relates to by clicking on a
                    codename
                    tile. If the detective guesses correctly then they will be
                    able to continue guessing until they have guessed the amount
                    specified by the spymaster. If the detective's guesses are
                    all
                    correct, they will get a free guess which they can choose to
                    make or pass. If the detective guesses incorrectly it
                    becomes
    ...
    for (let i = 0; i < 10; i++) {
      table.push(
          <div
              ...
              className={'amount' +
              (i <= this.state.wordsLeft ? '' : ' disabled')}
          >
            {i}
          </div>,
      );
        ...
        <div className="chat">
          <div className="chat-container">
            {this.state.log.map(hint => {
              return (
                  <div key={hint.clue + hint.amt} className="clue" style={{color: hint.sender}}>
                    {hint.clue} : {hint.amt}
                  </div>
              );
            })}
            <div className={'hintSubmission'}>
              <input placeholder="Clue" onChange={(e) => this.setState(
                  {clue: e.target.value})}
                     id="msg"/>
